...
public class ConfigBuilder
{
	...
	public static void main( final String[] args ) throws URISyntaxException,
			...
			InstantiationException, IllegalAccessException, MissingAnnotation, ParseException
	{
...
//		final String urlStr = "jdbc:j4s?builder=org.xenei.jdbc4sparql.sparql.builders.RDFSBuilder&type=turtle:file:"
		final String urlStr = String.format("jdbc:j4s?builder=%s&type=turtle:file:%s", cmd.getOptionValue("b"), outFile.getCanonicalPath());
		...
		System.out.println("Writing configuration to " + cmd.getOptionValue("o"));
	...
	private static Options getOptions()
	{
		...
		opt = new Option("b", "builder", true, "The schema builder class to use");
		opt.setRequired(true);
		retval.addOption( opt );
		
		opt = new Option("f", "file", true, "A file to add to the input.  Multiple files may be used");
		opt.setRequired(true);
		retval.addOption( opt );
		...
		//retval.addOption( "d", "dataset", false, "The dataset producer class to use (defaults to TDBDataset)");
		
		//retval.addOption("c", "catalog", true, "The name for the catalog being built." );
		return retval;
	}
	...
	private static void help()
	{
		...
		formatter.printHelp( "ConfigBuilder", getOptions() );
	}
}
