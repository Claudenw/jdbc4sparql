 ...
 * 
 * This is the name that included the Catalog, Schema, Table, and Column name segments.
 *
 */
public interface FQName extends GUIDObject {

	/**
	 * A helper class for comparing FQNames.  Provides implementations of compare(), 
	 * equals() and hashCode().
	 *
	 */
	public static class Comparator implements java.util.Comparator<FQName> {
		@Override
		public int compare(FQName fqName1, FQName fqName2) {
			return new CompareToBuilder()
					...
					.append(fqName1.getColumn(), fqName2.getColumn()).toComparison();
		}

		/**
		 * Determines if 2 FQNames are equal.  A FQName is equal if all the name segments are equal.
		 * @param fqName1 the first FQName
		 * @param fqName2 the second FQName
		 * @return  true if equal, false if not
		 */
		public static boolean equals(Object fqName1, Object fqName2) {
			if (fqName1 instanceof FQName && fqName2 instanceof FQName) {
				return ((FQName)fqName1).getGUID().equals( ((FQName)fqName2).getGUID());
...
//						.append(bn1.getColumn(), bn2.getColumn()).isEquals();
		...
		/**
		 * Generates a common hash code for the FQName based on all the name segments.
		 * @param fqName
		 * @return
		 */
		public static int hashCode(FQName fqName) {
...
//					.append(fqName.getColumn()).toHashCode();
		...
		public static String makeGUID(String catalog, String schema, String table, String column) {
