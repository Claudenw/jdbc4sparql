...
public class SparqlQueryBuilder {
    ...
    /**
     * Add the column to the query. Column must be in a table that has already been
     * added to the query.
     *
     * @param cName
     *            The column name.
    ...
    /**
     * Adds the the expression as a variable to the query. As a variable the result
    ...
    public Query build() throws SQLDataException {
        ...
        if (!isBuilt) {
            ...
            /*
             * apply the type filters to each subpart. may change variable names in result
             */
            ...
            final List<SortCondition> lsc = query.getOrderBy();
            if (lsc != null) {
    ...
    /**
     * Get the list of query columns for the variables in the project vars. If the
     * var is not found in the query info then null is returned. Such values are
     * calculated in a return function.
     *
     * @return
     */
    public List<QueryColumnInfo> getResultColumns() {
        final List<QueryColumnInfo> retval = new ArrayList<>();
     ...
     *
     * The aggregator will be registered as a column with in the virtual table for
     * the virtual schema of the current catalog.
     *
     * @param agg
     *            The aggregator to register.
     * @param type
     *            they SQL data type of the aggregator result.
     * @param alias
     *            the alias for the aggregator.
    ...
    public ExprAggregator register(final Aggregator agg, final int type, final String alias) throws SQLException {
        ...
        final ColumnName columnName = ColumnName.getNameInstance( catalog.getShortName(), VirtualSchema.NAME,
     ...
     * @param type
     *            the SQL return type for the function
     * @return the QueryColumnInfo for the function.
     */
    public QueryColumnInfo registerFunction(final ColumnName funcName, final int type) {
    ...
    public QueryColumnInfo registerFunctionColumn(final ColumnName columnName, final int type) {

        QueryTableInfo tableInfo = infoSet.getTable( columnName.getTableName() );
        Column column = null;
        if (tableInfo == null) {
            final FQName fqName = columnName.getFQName();
    ...
    /**
     * Sets all the columns for all the tables currently defined. This method should
     * be called after all tables have been added to the querybuilder.
     *
     * @throws SQLException
     */
    public void setAllColumns() throws SQLException {
        ...
        final QueryItemCollection<QueryColumnInfo, Column, ColumnName> colInfoList = new QueryItemCollection<>();
        ...
        for (final QueryColumnInfo columnInfo : colInfoList) {
            if (!ItemNameMatcher.contains( columnsInUsing, columnInfo.getName(), NameSegments.FFFT )) {
                ...
                while ((colInfoList.count( columnInfo.getName(), segs ) > 1) && !segs.isCatalog()) {
                    if (segs.isSchema()) {
    ...
    /**
     * Returns the table name if only one exists in the query. If multiple tables
     * exist or no table has been added return null.
     *
    ...
    private class NodeMapper implements NodeTransform {
        Map<Node, QueryColumnInfo> vars = new HashMap<>();

        NodeMapper() {
            final VarExprList varExpLst = query.getProject();
            for (final Var v : varExpLst.getVars()) {
                vars.put( v.asNode(), infoSet.findColumnByGUID( v ) );
                final Expr exp = varExpLst.getExpr( v );
                if (exp != null) {
                    exp.applyNodeTransform( new NodeTransform() {
                        ...
                        public Node apply(final Node t) {
                            if (t.isVariable()) {
                                final QueryColumnInfo qci = infoSet.findColumnByGUID( Var.alloc( t.getName() ) );
                                if (qci == null) {
                                    SparqlQueryBuilder.LOG.info( String.format( "Can not find %s in infoSet columns -- will not map it",
                                            t ) );
                                } else {
                                    vars.put( t, qci );
                                }
         ...
         */
        private QueryColumnInfo getColumn(final Var v) {
            return vars.values().stream().filter( qci -> (qci != null) && qci.getVar().equals( v ) ).findFirst()
                    .orElse( null );
        }

        public Var transform(final Var t) {
            final QueryColumnInfo retval = vars.get( t.asNode() );
            return retval == null ? t : retval.getVar();
        }

        @Override
        public Node apply(final Node t) {
            final QueryColumnInfo retval = vars.get( t );
            return retval == null ? t : retval.getVar().asNode();
        }

        public void map(final VarExprList vel) {
            for (int i = 0; i < vel.getVars().size(); i++) {
                final Var v = vel.getVars().get( i );
                final Var r = nodeMapper.transform( v );
                if (!v.equals( r )) {
                    vel.getVars().set( i, r );
                }
        ...
        public <T extends Expr> void map(final List<T> exprLst) {
